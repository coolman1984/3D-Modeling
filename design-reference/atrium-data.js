...
export function plan(items, sel = [], findings = [], opts = {}) {
  ...
  items.forEach((it) => {
    ...
    shapes.push({ id: it.id, it, l: pw(it.x - dm.w / 2), t: pd(it.y - dm.d / 2), w: pw(dm.w), h: pd(dm.d), r: ty.round ? '50%' : '1px', bg, bd, fg, label, fs: it.t === 'rt180' ? 'max(8px, .75cqw)' : 'max(9px, .85cqw)', locked: !!it.locked, lockDisp: it.locked && s ? 'flex' : 'none' });
    chairs(it).forEach((c, i) => seats.push({ key: it.id + i, l: pw(c.x - 22), t: pd(c.y - 22), w: pw(44), h: pd(44), a: c.a, bd: s ? `1px solid ${ACC}` : e === 'error' ? `1px dashed ${ERR}` : '1px solid #57534c', bg: s ? '#e6ecfc' : '#fdfcfa' }));
    if (e) badges.push({ id: it.id, l: pw(it.x + dm.w / 2), t: pd(it.y - dm.d / 2), bg: e === 'error' ? ERR : '#fff', fg: e === 'error' ? '#fff' : WARN, bd: e === 'error' ? ERR : WARN, icon: e === 'error' ? 'lucide:x' : 'lucide:triangle-alert' });
  });
