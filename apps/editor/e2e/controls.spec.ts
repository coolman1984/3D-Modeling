...
test('select many with a box, move with the keyboard as one step, raise, copy and turn', async ({ page }) => {
  const id = await newProject(page, 'قاعة التحكم');
  ...
  await expect(page.getByLabel('العنصر المختار')).toContainText('٣ عناصر مختارة');
  ...
  await expect(page.getByLabel('فوق الأرض')).toHaveValue('5');
...
test('precision settings change the step and are remembered; Alt drags slowly; right button pans', async ({ page }) => {
  const id = await newProject(page, 'قاعة الدقة');
  await page.locator('[data-add="chair"]').click();
  await page.getByRole('tab', { name: 'الدقة والسرعة' }).click();
...
test('in 3D an item slides over the floor and Shift+drag raises it', async ({ page }) => {
  const id = await newProject(page, 'قاعة المجسم', 10, 8);
...
test('the client report shows the plan, the 3D picture, the quantities and the issues, ready to print', async ({ page }) => {
  await newProject(page, 'قاعة العميل', 12, 9);
  ...
  await expect(report.locator('h1')).toHaveText('قاعة العميل');
  await expect(page.getByTestId('report-seats')).toHaveText('٤');
  await expect(page.getByTestId('report-bom').locator('tbody tr')).toHaveCount(2);
  await expect(page.getByTestId('report-bom')).toContainText('١٫٨ م × ٨٠ سم × ٧٥ سم');
  ...
  await expect(page.getByRole('button', { name: 'اطبع التقرير' })).toBeHidden();
