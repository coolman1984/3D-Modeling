...
import { packOf, PACKS, rackOf, SHAPES, shapeOf, stationOf, type RuleResult } from '@space-planner/starter';
import {
  AlignBottom,
...
import { restaurantFacts } from './Restaurant.js';
...
export function PropertiesPanel({
  ...
  if (items.length === 1) return <OneItem project={project} id={items[0]!.id} controls={controls} issues={issues} dispatch={dispatch} onEditType={onEditType} onShow3D={onShow3D} onFocusIssue={onFocusIssue} cargo={activity.pack === 'container'} warehouse={activity.pack === 'warehouse'} factory={activity.pack === 'factory'} bayTravel={bayTravel} />;
...
function ProjectSummary({ project, metrics, activity, summary, onOpenReview }: { project: Project; metrics: Metrics; activity: Activity; summary: ReviewSummary; onOpenReview: () => void }) {
  ...
  const facts: Array<[string, ReactNode]> = activity.pack === 'container' ? containerFacts(project) : activity.pack === 'warehouse' ? warehouseFacts(project) : activity.pack === 'factory' ? factoryFacts(project) : activity.pack === 'depot' ? depotFacts(project) : activity.pack === 'restaurant' ? restaurantFacts(project, activity.style) : [
...
function OneItem({
  ...
  return (
    <div aria-label="Selected item">
      ...
      {factory && stationOf(definition) && <FlowGroup project={project} item={item} dispatch={dispatch} />}
