                  ...
                  <a href={`#/p/${p.id}`} className="proj-cols">
                    ...
                    <span style={{ minWidth: 0 }}>
                      ...
                      <span className="proj-sub">
                        Revision {p.revision}
                        {p.variantOf && <span data-variant-of={p.variantOf}> · Variant of {projects?.find((b) => b.id === p.variantOf)?.name ?? 'another plan'}</span>}
                      </span>
                    </span>
                    <span className="opt">
                      {pack?.label ?? '—'}
              ...
              return (
                <a key={p.id} href={`#/p/${p.id}`} className="proj-card" data-project={p.id}>
                  ...
                  <span className="proj-sub" style={{ fontSize: 13, marginTop: 6 }}>
                    {[p.variantOf ? `Variant of ${projects?.find((b) => b.id === p.variantOf)?.name ?? 'another plan'}` : null, card ? packOf(card.activity.pack).label : null, room ? `${formatMetres(room.maxX - room.minX)} × ${formatMetres(room.maxY - room.minY)} m` : null, plural(p.itemCount, 'item')].filter(Boolean).join(' · ')}
                  </span>
