...
export const TOOLS: readonly ToolDef[] = [
  ...
  },
  {
    name: 'create_variant',
    description:
      'Make a linked alternative of a project so an idea can be tested without touching the approved plan. The variant has its own history and can later be compared or adopted.',
    inputSchema: {
      type: 'object',
      properties: { project_id: projectId, name: { type: 'string', description: 'Short name for the alternative.' } },
  ...
  },
  {
    ...
    description: 'Compare a project and its variants side by side using validation results and activity-specific figures. Nothing changes.',
    inputSchema: { type: 'object', properties: { project_id: projectId }, required: ['project_id'], additionalProperties: false },
    run: (ctx, input) => {
      const project = load(ctx, input);
      ...
      return rows
        .map((row) => `${row.base ? 'BASE ' : ''}${row.project.id} "${row.project.name}" rev ${row.project.revision}: ${row.errors} errors, ${row.warnings} warnings, ${row.rulesFailed} rules failed, ${row.rulesUnknown} unknown; ${row.figures.map((figure) => `${figure.label} ${figureText(figure)}`).join(', ')}`)
        .join('\n');
    },
  },
  {
    name: 'adopt_variant',
    description: 'Adopt a variant into its approved base as one new validated, undoable revision. Use only after the person asks to adopt that alternative.',
