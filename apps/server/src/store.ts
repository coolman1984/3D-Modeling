...
export class Store extends EventEmitter<StoreEvents> {
  ...
  constructor(path: string) {
    ...
    this.db.exec(SCHEMA);
    // Added with variants: existing databases gain the column; old rows are ordinary projects.
    const columns = this.db.prepare('PRAGMA table_info(projects)').all() as Array<{ name: string }>;
    if (!columns.some((c) => c.name === 'variant_of')) this.db.exec('ALTER TABLE projects ADD COLUMN variant_of TEXT');
  ...
  summary(id: string): ProjectSummary | null {
    const row = this.db.prepare('SELECT id, name, revision, item_count, created_at, updated_at, variant_of FROM projects WHERE id = ?').get(id) as Record<string, string | number | null> | undefined;
    return row ? toSummary(row) : null;
  }

  /**
   * A new alternative of a project: a copy linked to its base (a variant of a variant links to
   * the same base), with its own history. The base, the approved plan, is not touched.
   */
  ...
  /** The base first, then its variants, oldest first. */
  familyOf(id: string): ProjectSummary[] {
    const info = this.summary(id);
    if (!info) return [];
    const base = info.variantOf ?? id;
    const variants = (this.db.prepare('SELECT id, name, revision, item_count, created_at, updated_at, variant_of FROM projects WHERE variant_of = ? ORDER BY created_at, id').all(base) as Array<Record<string, string | number | null>>).map(toSummary);
    const head = this.summary(base);
    return head ? [head, ...variants] : variants;
  }

  /**
   * Make the base look like the variant, as one ordinary revision of the base built from core
   * commands (validated, undoable, in the history under the actor's name). The variant stays.
   */
  adoptVariant(variantId: string, actor: string): ApplyResult | { ok: false; status: 400 } {
  ...
  deleteProject(id: string): boolean {
    // Variants of a deleted base become ordinary projects; nothing else is lost.
...
function toSummary(r: Record<string, string | number | null>): ProjectSummary {
  return {
    id: String(r.id),
    name: String(r.name),
    revision: Number(r.revision),
    ...
    variantOf: r.variant_of === null || r.variant_of === undefined ? null : String(r.variant_of),
  };
}
